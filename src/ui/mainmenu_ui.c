...
void mainmenu_ui_shop_refresh(TigRect* rect)
{
    ...
    // DEV: Seed all unique items into the first trader for testing.
    // Placed after sub_463E20 calls so inventory clear doesn't wipe them.
    {
        static const struct { int bp; UniqueItemId uid; } test_uniques[] = {
            { BP_QUALITY_SWORD,         UNIQUE_BLADE_OF_ALBERICH  },
            { BP_OILED_THIEVES_LEATHER, UNIQUE_VEIL_OF_SHADOWS    },
            { BP_MACHINED_PLATEMAIL,    UNIQUE_IRONCLAD_COGPLATE  },
            { BP_MAGES_DAGGER,          UNIQUE_RING_OF_MEPHISTIS  },
            { BP_MAGES_STAFF,           UNIQUE_TULLIAN_FOCUS      },
        };
        int64_t target_obj = (substitute_inventory_obj != OBJ_HANDLE_NULL) ? substitute_inventory_obj : npc_obj;
        int64_t loc = obj_field_int64_get(target_obj, OBJ_F_LOCATION);
        for (int i = 0; i < (int)(sizeof(test_uniques) / sizeof(test_uniques[0])); i++) {
            int64_t proto_obj = sub_4685A0(test_uniques[i].bp);
            int64_t item_obj;
            if (object_create(proto_obj, loc, &item_obj)) {
                item_rarity_apply_unique(item_obj, test_uniques[i].uid);
                item_rarity_identify(item_obj);
                item_transfer(item_obj, target_obj);
            }
        }
    }

    // DEV: Seed rarity test items directly into PC inventory for loot highlight testing.
    // Drop these on the ground to see colored hover rings.
    {
        static const struct { int bp; ItemRarity rarity; } test_items[] = {
            { BP_SWORD,         ITEM_RARITY_UNCOMMON },
            { BP_AXE,           ITEM_RARITY_RARE     },
            { BP_MACE,          ITEM_RARITY_EPIC     },
            { BP_LEATHER_ARMOUR, ITEM_RARITY_UNCOMMON },
            { BP_CHAINMAIL,     ITEM_RARITY_RARE     },
        };
        int64_t loc = obj_field_int64_get(pc_obj, OBJ_F_LOCATION);
        for (int i = 0; i < (int)(sizeof(test_items) / sizeof(test_items[0])); i++) {
            int64_t proto_obj = sub_4685A0(test_items[i].bp);
            int64_t item_obj;
            if (object_create(proto_obj, loc, &item_obj)) {
