...
void mainmenu_ui_new_char_create(void)
{
    ...
    // CE: NG+ bonus points - added here so player sees them in the character roller.
    {
        int ng = ng_plus_get_level();
        if (ng > 0) {
            stat_base_set(player_create_info.obj, STAT_UNSPENT_POINTS,
                stat_base_get(player_create_info.obj, STAT_UNSPENT_POINTS) + ng);
        }
    }
...
bool mainmenu_ui_pregen_char_execute(int btn)
{
    ...
    // CE: NG+ bonus points for pregen characters.
    {
        int ng = ng_plus_get_level();
        if (ng > 0) {
            stat_base_set(player_create_info.obj, STAT_UNSPENT_POINTS,
